...
public class MegaBook : MonoBehaviour
{
	...
	void Update()
	{
		...
		double step = 1.0f / (double)pagecount;
		...
		if ( page == 1 )
		{
			pf1.turn = Mathf.Clamp((float)turn * 100.0f, 0.0f, 100.0f);
			pf2.turn = 0.0f;
			pf3.turn = 0.0f;
		}
		else
		{
			if ( page == pagecount - 2 )
			{
				pf1.turn = 100.0f;
				pf2.turn = 100.0f;
				pf3.turn = Mathf.Clamp((float)turn * 100.0f, 0.0f, 100.0f); //(float)turn * 100.0f;
			}
			else
			{
				if ( page == 0 )
				...
				else
				{
					...
					{
						pf1.turn = 100.0f;
						pf2.turn = Mathf.Clamp((float)turn * 100.0f, 0.0f, 100.0f); //(float)turn * 100.0f;
						pf3.turn = 0.0f;
					}
				}
			}
	...
	void Interactive()
	{
		if ( !pageTurning )
		{
			...
			//bookalpha += Input.GetKey(KeyCode.D) * keysensi;
			//bookalpha -= (Input.GetKey(KeyCode.D) ? 1 : 0) * keysensi;
			if ( Input.GetKeyDown(KeyCode.A) ) StartCoroutine(FlipToPage(currentPage - 1));
			if ( Input.GetKeyDown(KeyCode.D) ) StartCoroutine(FlipToPage(currentPage + 1));
	...
	{
		pageTurning = true;
		float totalPageCount = (float)((pages.Count + 4) / 2);
		Debug.Log(totalPageCount);
		float target = (float)page / totalPageCount * 100.0f;
		target = Mathf.Clamp(target, 0.0f, 100.0f);
		while ( bookalpha != target )
		{
			bookalpha = Mathf.MoveTowards(bookalpha, target, keysensi * Time.deltaTime);
			yield return 0;
		}
		currentPage = page;
		pageTurning = false;
