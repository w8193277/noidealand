...
public class ThirdPersonCameraController : MonoBehaviour {
	...
	void LateUpdate (){
		...
		if(mouseSideButton && Input.GetAxis("Vertical") != 0 ) { mouseSideButton = false; }
		...
		if (GUIUtility.hotControl == 0){
			if (Input.GetMouseButton(0) || Input.GetMouseButton(1)){
				...
				if (allowMouseInputX) { xDeg += Input.GetAxis ("Mouse X") * xSpeed * 0.02f; }
				
				else { RotateBehindTarget(); }
				
				if (allowMouseInputY) { yDeg -= Input.GetAxis ("Mouse Y") * ySpeed * 0.02f; }
